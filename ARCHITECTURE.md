...
- ✅ **代码清晰、结构简单**：适合教学，不追求工业级复杂度
- ✅ **架构简化**：后端直接托管前端页面，只需启动一个服务
- ✅ **前后端分离**：FastAPI 后端 + 原生 HTML/JS 前端（但由后端统一托管）
- ✅ **自动适配部署环境**：前端自动根据访问地址适配后端和 ComfyUI 地址
- ✅ **完整功能闭环**：从输入 prompt 到展示生成图片，包含实时进度监听
- ✅ **现代化 UI 设计**：紫色渐变风格，响应式布局，专业交互体验
- ✅ **模板提示词功能**：预设 4 种风格模板，一键快速开始
...
- **一键填充**：点击模板卡片，自动填充提示词到输入框
- **视觉化展示**：每个模板配有渐变色彩缩略图和详细描述
...
**方式一：手动输入提示词**
1. 在「输入提示词」区域输入 prompt（例如：`一只可爱的猫咪坐在月亮上,星空背景,梦幻风格`）
2. 观察字符计数实时更新
3. 点击「生成图片」按钮（带星星图标）
4. 观察状态变化：
   - 「开始调用后端 /generate...」
   - 「已连接 WebSocket，等待生成进度...」
   - 「执行进度: 5 / 9 (55.6%)」
   - 「执行完成，开始获取历史记录 / 图片...」
   - 「图片生成完成」
5. 生成结果区域自动显示，图片淡入展示
...
**方式二：使用模板提示词**
1. 在「模板提示词」区域浏览 4 种预设模板
2. 点击任意模板卡片（奇幻风景、赛博朋克城市、动漫角色、抽象艺术）
3. 提示词自动填充到输入框
4. 可以在此基础上修改或直接点击「生成图片」

**其他功能**
- **清空按钮**：快速清空输入框内容
- **调试日志**：点击展开/折叠，查看技术细节（开发调试用）
...
**优势**：
- ✅ 只需启动一个服务（后端），架构更简单
- ✅ 无需单独的静态服务器（如 nginx、python -m http.server）
- ✅ 前后端同端口，避免跨域配置问题

---

### 分布式部署（前端、后端、ComfyUI 在不同机器）

**不推荐**：当前架构假设前端、后端、ComfyUI 在同一台机器或同一内网。
...
### 2. CORS 问题与 WebSocket 连接

- **HTTP 请求**：必须通过后端转发（`/history`、`/image`），避免浏览器 CORS 拦截
- **WebSocket 连接**：
  - **前端直连 ComfyUI**：`ws://{host}:8188/ws?clientId=...`
  - **不经过后端**：WebSocket 协议不受 CORS 限制，可以直接连接
  - **优势**：减少延迟，实时性更好，简化架构
  - **注意**：确保 ComfyUI WebSocket 服务可被前端访问（同一网络或配置防火墙）
- **后端 CORS**：已配置 `allow_origins=["*"]`（仅限 Demo，生产环境需限制）

### 3. 端口要求

- **后端**：8000（可在代码中修改，同时托管前端页面）
- **ComfyUI**：8188（ComfyUI 默认端口）
- **前端**：由后端通过 `/static` 路径提供，无需单独端口

### 4. UI 交互说明

- **生成结果区域**：默认隐藏，只有在图片生成完成后才会显示（带淡入动画）
- **模板提示词**：点击模板卡片会自动填充到输入框，可以在此基础上修改
- **字符计数**：实时显示输入框字符数，帮助用户控制提示词长度
- **调试日志**：默认折叠，点击展开查看技术细节，保持界面简洁
...
- **前端日志**：查看浏览器 F12 Console 和页面「调试日志」区域
- **ComfyUI 日志**：查看 ComfyUI 启动终端的输出

---

## 📄 许可证

本项目为教程级 Demo，代码清晰、结构简单，适合学习和教学使用。

---

## 🎓 教学要点

1. **前后端分离**：前端只负责 UI 和用户交互，后端负责业务逻辑和 ComfyUI 集成
2. **WebSocket 实时通信**：
   - 前端直连 ComfyUI WebSocket，不经过后端
   - WebSocket 不受 CORS 限制，适合实时推送场景
   - 减少延迟，提升用户体验
3. **CORS 处理策略**：
   - HTTP 请求通过后端转发（解决 CORS）
   - WebSocket 直连（不受 CORS 限制）
   - 混合架构：根据协议特性选择最优路径
4. **workflow 动态替换**：使用 `deepcopy` 避免修改原始 workflow
5. **自动适配环境**：前端自动检测主机地址，方便部署
6. **现代化 UI 设计**：渐变色彩、响应式布局、动画过渡、专业交互体验
7. **用户体验优化**：条件显示（结果区域默认隐藏）、模板快速开始、实时反馈
...
**最后更新**：2025-12-23  
**更新内容**：
- ✅ **架构简化**：后端直接托管前端页面，无需单独的静态服务器
- ✅ **更新架构图**：反映后端通过 `/static` 路径托管前端，根路径重定向到前端页面
- ✅ **简化部署**：只需启动后端服务（8000 端口），前端由后端统一提供
- ✅ **更新使用说明**：访问方式改为 `http://{host}:8000/`，无需额外端口
- ✅ **修正架构图**：明确标注 WebSocket 是前端直连 ComfyUI，不经过后端
- ✅ **更新数据流说明**：详细说明 WebSocket 直连的设计和原因
- ✅ **完善注意事项**：添加 WebSocket 连接的网络要求说明
- ✅ **更新教学要点**：强调混合架构设计（HTTP 转发 + WebSocket 直连）
- 添加模板提示词功能说明
- 添加现代化 UI 设计说明
- 更新使用流程和交互说明
- 完善技术栈和文件说明
